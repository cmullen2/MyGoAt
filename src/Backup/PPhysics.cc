...
void PPhysics::FillPx(const GTreeParticle& tree, TH1* Hprompt, TH1* Hrandom)
{
    for (Int_t i = 0; i < tree.GetNParticles(); i++)
    {
    for (Int_t j = 0; j < GetTagger()->GetNTagged(); j++)
	{
        	FillPx(tree, i, j, Hprompt, Hrandom);
	}
    }
}

void PPhysics::FillPx(const GTreeParticle& tree, Int_t particle_index, TH1* Hprompt, TH1* Hrandom)
{
    for (Int_t i = 0; i < GetTagger()->GetNTagged(); i++)
	{
        	FillPx(tree, particle_index, i, Hprompt, Hrandom);
	}
}

void PPhysics::FillPx(const GTreeParticle& tree, Int_t particle_index, Int_t tagger_index, TH1* Hprompt, TH1* Hrandom)
{
	...
	if (GHistBGSub::IsPrompt(time)) Hprompt->Fill(missingp4.Px());
	if (GHistBGSub::IsRandom(time)) Hrandom->Fill(missingp4.Px());						
}
...
void PPhysics::FillPy(const GTreeParticle& tree, TH1* Hprompt, TH1* Hrandom)
{
    for (Int_t i = 0; i < tree.GetNParticles(); i++)
    {
    for (Int_t j = 0; j < GetTagger()->GetNTagged(); j++)
	{
        	FillPy(tree, i, j, Hprompt, Hrandom);
	}
    }
}

void PPhysics::FillPy(const GTreeParticle& tree, Int_t particle_index, TH1* Hprompt, TH1* Hrandom)
{
    for (Int_t i = 0; i < GetTagger()->GetNTagged(); i++)
	{
        	FillPy(tree, particle_index, i, Hprompt, Hrandom);
	}
}

void PPhysics::FillPy(const GTreeParticle& tree, Int_t particle_index, Int_t tagger_index, TH1* Hprompt, TH1* Hrandom)
{
    time = GetTagger()->GetTaggedTime(tagger_index) - tree.GetTime(particle_index);
    missingp4 = CalcMissingP4(tree, particle_index,tagger_index);

	if (GHistBGSub::IsPrompt(time)) Hprompt->Fill(missingp4.Py());
	if (GHistBGSub::IsRandom(time)) Hrandom->Fill(missingp4.Py());						
}




void PPhysics::FillPz(const GTreeParticle& tree, TH1* Hprompt, TH1* Hrandom)
{
    for (Int_t i = 0; i < tree.GetNParticles(); i++)
    {
    for (Int_t j = 0; j < GetTagger()->GetNTagged(); j++)
	{
        	FillPz(tree, i, j, Hprompt, Hrandom);
	}
    }
}

void PPhysics::FillPz(const GTreeParticle& tree, Int_t particle_index, TH1* Hprompt, TH1* Hrandom)
{
    for (Int_t i = 0; i < GetTagger()->GetNTagged(); i++)
	{
        	FillPz(tree, particle_index, i, Hprompt, Hrandom);
	}
}

void PPhysics::FillPz(const GTreeParticle& tree, Int_t particle_index, Int_t tagger_index, TH1* Hprompt, TH1* Hrandom)
{
    time = GetTagger()->GetTaggedTime(tagger_index) - tree.GetTime(particle_index);
    missingp4 = CalcMissingP4(tree, particle_index,tagger_index);

	if (GHistBGSub::IsPrompt(time)) Hprompt->Fill(missingp4.Pz());
	if (GHistBGSub::IsRandom(time)) Hrandom->Fill(missingp4.Pz());						
}
...
void PPhysics::FillE(const GTreeParticle& tree, TH1* Hprompt, TH1* Hrandom)
{
    for (Int_t i = 0; i < tree.GetNParticles(); i++)
    {
    for (Int_t j = 0; j < GetTagger()->GetNTagged(); j++)
	{
        	FillE(tree, i, j, Hprompt, Hrandom);
	}
    }
}

void PPhysics::FillE(const GTreeParticle& tree, Int_t particle_index, TH1* Hprompt, TH1* Hrandom)
{
    for (Int_t i = 0; i < GetTagger()->GetNTagged(); i++)
	{
        	FillE(tree, particle_index, i, Hprompt, Hrandom);
	}
}

void PPhysics::FillE(const GTreeParticle& tree, Int_t particle_index, Int_t tagger_index, TH1* Hprompt, TH1* Hrandom)
{
    time = GetTagger()->GetTaggedTime(tagger_index) - tree.GetTime(particle_index);
    missingp4 = CalcMissingP4(tree, particle_index,tagger_index);

	if (GHistBGSub::IsPrompt(time)) Hprompt->Fill(missingp4.E());
	if (GHistBGSub::IsRandom(time)) Hrandom->Fill(missingp4.E());						
}
...
void PPhysics::FillPx(const GTreeParticle& tree, GH1* gHist, Bool_t TaggerBinning)
{
	for (Int_t i = 0; i < tree.GetNParticles(); i++)
	{
        for (Int_t j = 0; j < GetTagger()->GetNTagged(); j++)
		{
			FillPx(tree, i, j, gHist, TaggerBinning);
		}
	}
}


void PPhysics::FillPx(const GTreeParticle& tree, Int_t particle_index, GH1* gHist, Bool_t TaggerBinning)
{
    for (Int_t i = 0; i < GetTagger()->GetNTagged(); i++)
	{
...
void PPhysics::FillPx(const GTreeParticle& tree, Int_t particle_index, Int_t tagger_index, GH1* gHist, Bool_t TaggerBinning)
{
    // Is tagger channel rejected by user?
    if(GetTagger()->GetTaggedChannel(tagger_index) < TC_cut_min) return;
    if(GetTagger()->GetTaggedChannel(tagger_index) > TC_cut_max) return;
...
void PPhysics::FillPy(const GTreeParticle& tree, GH1* gHist, Bool_t TaggerBinning)
{
	for (Int_t i = 0; i < tree.GetNParticles(); i++)
	{
        for (Int_t j = 0; j < GetTagger()->GetNTagged(); j++)
		{
			FillPy(tree, i, j, gHist, TaggerBinning);
		}
	}
}


void PPhysics::FillPy(const GTreeParticle& tree, Int_t particle_index, GH1* gHist, Bool_t TaggerBinning)
{
    for (Int_t i = 0; i < GetTagger()->GetNTagged(); i++)
	{
        FillPy(tree, particle_index, i, gHist, TaggerBinning);
	}
}



void PPhysics::FillPy(const GTreeParticle& tree, Int_t particle_index, Int_t tagger_index, GH1* gHist, Bool_t TaggerBinning)
{
    // Is tagger channel rejected by user?
    if(GetTagger()->GetTaggedChannel(tagger_index) < TC_cut_min) return;
    if(GetTagger()->GetTaggedChannel(tagger_index) > TC_cut_max) return;
...
void PPhysics::FillPz(const GTreeParticle& tree, GH1* gHist, Bool_t TaggerBinning)
{
	for (Int_t i = 0; i < tree.GetNParticles(); i++)
	{
        for (Int_t j = 0; j < GetTagger()->GetNTagged(); j++)
		{
			FillPz(tree, i, j, gHist, TaggerBinning);
		}
	}
}


void PPhysics::FillPz(const GTreeParticle& tree, Int_t particle_index, GH1* gHist, Bool_t TaggerBinning)
{
    for (Int_t i = 0; i < GetTagger()->GetNTagged(); i++)
	{
        FillPz(tree, particle_index, i, gHist, TaggerBinning);
	}
}



void PPhysics::FillPz(const GTreeParticle& tree, Int_t particle_index, Int_t tagger_index, GH1* gHist, Bool_t TaggerBinning)
{
    // Is tagger channel rejected by user?
    if(GetTagger()->GetTaggedChannel(tagger_index) < TC_cut_min) return;
    if(GetTagger()->GetTaggedChannel(tagger_index) > TC_cut_max) return;
...
void PPhysics::FillE(const GTreeParticle& tree, GH1* gHist, Bool_t TaggerBinning)
{
	for (Int_t i = 0; i < tree.GetNParticles(); i++)
	{
        for (Int_t j = 0; j < GetTagger()->GetNTagged(); j++)
...
void PPhysics::FillE(const GTreeParticle& tree, Int_t particle_index, GH1* gHist, Bool_t TaggerBinning)
{
    for (Int_t i = 0; i < GetTagger()->GetNTagged(); i++)
	{
...
void PPhysics::FillE(const GTreeParticle& tree, Int_t particle_index, Int_t tagger_index, GH1* gHist, Bool_t TaggerBinning)
{
    // Is tagger channel rejected by user?
    if(GetTagger()->GetTaggedChannel(tagger_index) < TC_cut_min) return;
    if(GetTagger()->GetTaggedChannel(tagger_index) > TC_cut_max) return;
